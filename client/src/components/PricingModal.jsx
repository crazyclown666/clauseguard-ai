  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-fadeIn">
      <div className="glass-panel w-full max-w-4xl rounded-3xl border border-slate-700 shadow-2xl p-6 sm:p-8 relative max-h-[95vh] overflow-y-auto">
        ...
        {/* Security badge & Stripe Config Drawer */}
        <div className="pt-2 border-t border-slate-800/80">
          <div className="flex items-center justify-between text-xs text-slate-400">
            <div className="flex items-center space-x-1.5">
              <Shield className="w-3.5 h-3.5 text-brand-400" />
              <span>256-Bit Encrypted & Instant Access Guarantee</span>
            </div>

            <button
              onClick={() => setShowConfig(!showConfig)}
              className="flex items-center space-x-1 text-slate-500 hover:text-slate-300 transition-colors text-[11px]"
            >
              <Settings className="w-3 h-3" />
              <span>Owner Payment Link Settings</span>
              {showConfig ? <ChevronUp className="w-3 h-3" /> : <ChevronDown className="w-3 h-3" />}
            </button>
          </div>

          {/* Config Drawer for Owner to paste custom Stripe links anytime */}
          {showConfig && (
            <div className="mt-4 p-4 rounded-xl bg-slate-900/90 border border-slate-800 space-y-3 animate-fadeIn">
              <div className="text-xs font-bold text-slate-200">
                ⚙️ Custom Stripe Payment Links (Optional)
              </div>
              <p className="text-[11px] text-slate-400">
                Paste your custom Stripe Payment Links below to route payments directly to your Stripe checkout pages:
              </p>
