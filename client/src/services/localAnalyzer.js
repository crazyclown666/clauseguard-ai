/**
 * Intelligent Client-Side Contract Analyzer
 * Ensures ClauseGuard AI functions seamlessly even on static hosting (e.g. GitHub Pages)
...
export function analyzeContractLocally(text, customTitle = '') {
  const lower = text.toLowerCase();
  const clauses = [];
  let scorePenalty = 0;
  const redFlags = [];
  ...
  return {
    isMock: true,
    documentTitle: customTitle || detectDocumentType(text),
